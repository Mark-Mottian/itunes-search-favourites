 ...
 *
 * Keeping the limits in constants avoids "magic numbers" being repeated in the form logic.
 */
...
function SearchForm({ onSearch, isSearching, tokenReady }) {
  ...
  const hasSearchError = Boolean(formik.errors.searchTerm);
  ...
  function validateSearchForm(values) {
    ...
    /*
     * Block search terms that are too short to be useful.
     */
    if (
      trimmedSearchTerm &&
      trimmedSearchTerm.length < MIN_SEARCH_TERM_LENGTH
    ) {
      errors.searchTerm = `Search term must be at least ${MIN_SEARCH_TERM_LENGTH} characters.`;
    }
          ...
          <Row className="g-3 align-items-start">
            ...
            <Col md={2}>
              ...
              <Button
                type="submit"
                className="w-100"
                disabled={isSearching || !tokenReady || hasSearchError}
              >
